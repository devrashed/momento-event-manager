...
class UEM_Post_Types {
	...
	///order_id
	public static function webcu_get_order_ids_by_product_id($product_id) {
		global $wpdb;
	...
	public static function webcu_show_event_columns_value( $column, $post_id ) {
		...
		if ( $column == 'booking_seat' ) {
			...
			$refund = count($refund_ids);
			$total_order = $total_sold - $refund;
			echo esc_html( $total_order );
			...
			$refund = count($refund_ids);
			$available_seats = $refund + $remaining;

...
class UEM_WooCommerce {
	...
	public static function init() {
		...
		add_action( 'wp_head', array( __CLASS__, 'webcu_add_attendee_styles' ) );
	...
	public static function webcu_save_attendee_data_to_order( $order_id ) {
		...
		if ( isset( $_POST['webcu_attendee'] ) && is_array( $_POST['webcu_attendee'] ) ) {
			...
			for ( $i = 1; $i <= $total_quantity; $i++ ) {
				...
				foreach ( $webcu_fields as $field ) {
					if ( isset( $_POST['webcu_attendee'][$field][$i-1] ) ) {
						$attendee[$field] = sanitize_text_field( $_POST['webcu_attendee'][$field][$i-1] );
					} elseif ( isset( $_POST['webcu_attendee'][$field] ) && $i === 1 ) {
						// For single attendee forms
						$attendee[$field] = sanitize_text_field( $_POST['webcu_attendee'][$field] );
					}
				}

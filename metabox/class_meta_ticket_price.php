...
class class_meta_ticket_price{ 
    ...
    public function wtmem_ticket_price_sections($post) {  
           ...
           // Get saved data
            $registration_enabled = get_post_meta($post->ID, '_wtmem_tk_registration_enabled', true);
            $all_tickets = get_post_meta($post->ID, '_wtmem_tk_tickets', true);
            $advanced_toggle_state = get_post_meta($post->ID, '_wtmem_tk_advanced_toggle', true);
            ...
            $toggle_class = $registration_enabled == '1' ? 'wtmem_tk_on' : '';
            $section_style = $registration_enabled == '1' ? '' : 'display:none;';
            $advanced_toggle_class = $advanced_toggle_state == '1' ? 'wtmem_tk_on' : '';
            $advanced_column_style = $advanced_toggle_state == '1' ? '' : 'display:none;';    
        ?>
        <div class="wtmem_tk_container">
          <div class="wtmem_tk_shortcode-box">
            ...
            <div class="wtmem_tk_shortcode">[event-add-cart-section event="<?php echo esc_attr($post->ID); ?>"]</div>
            <div class="wtmem_tk_registration-row">
              <div style="font-size:13px;color:#666"><?php echo esc_html__('Registration Off/On:', 'ultimate-event-manager') ?></div>
              <div id="wtmem_tk_registrationToggle" class="wtmem_tk_toggle <?php echo esc_attr($toggle_class); ?>">
                <span class="wtmem_tk_knob"></span>
              </div>
          ...
          <div id="wtmem_tk_registrationSection" style="<?php echo esc_attr($section_style); ?>">
            ...
            <div class="wtmem_tk_controls-row">
              <label class="wtmem_tk_switch-label"> <?php echo esc_html__('Show Advanced Column:', 'ultimate-event-manager') ?></label>
              <div id="wtmem_tk_advancedToggle" class="wtmem_tk_toggle <?php echo esc_attr($advanced_toggle_class); ?>">
                <span class="wtmem_tk_knob"></span>
              </div>
              <input type="hidden" id="wtmem_tk_advanced_toggle" name="wtmem_tk_advanced_toggle" value="<?php echo esc_attr($advanced_toggle_state); ?>"  >
            </div>
            
            <table class="wtmem_tk_ticket-table" id="wtmem_tk_ticketTable">
              <thead>
                <tr>
                  ...
                  <th class="wtmem_tk_advanced" style="<?php echo esc_attr($advanced_column_style); ?>"><?php echo esc_html__('Default Qty', 'ultimate-event-manager') ?></th>
                  <th class="wtmem_tk_advanced" style="<?php echo esc_attr($advanced_column_style); ?>"><?php echo esc_html__('Reserve Qty', 'ultimate-event-manager') ?></th>

                  <th class="wtmem_tk_advanced" style="<?php echo esc_attr($advanced_column_style); ?>"><?php echo esc_html__('Sale Start Date', 'ultimate-event-manager') ?></th>
                  <th class="wtmem_tk_advanced" style="<?php echo esc_attr($advanced_column_style); ?>"><?php echo esc_html__('Sale Start Time', 'ultimate-event-manager') ?></th>
              ...
              <tbody>
                <?php if (!empty($tickets)) : ?>
                  <?php foreach ($tickets as $key => $ticket) : ?>
                    <tr class="wtmem_tk_ticket-row">
                      <td><input type="text" name="wtmem_tk_regular_tickets[<?php echo esc_attr($key); ?>][name]" value="<?php echo esc_attr($ticket['name']); ?>"></td>
                      <td><input type="text" name="wtmem_tk_regular_tickets[<?php echo esc_attr($key); ?>][desc]" value="<?php echo esc_attr($ticket['desc']); ?>" placeholder="Short description"></td>
                      <td><input type="number" name="wtmem_tk_regular_tickets[<?php echo esc_attr($key); ?>][price]" value="<?php echo esc_attr($ticket['price']); ?>"></td>
                      <td><input type="number" name="wtmem_tk_regular_tickets[<?php echo esc_attr($key); ?>][capacity]" value="<?php echo esc_attr($ticket['capacity']); ?>" placeholder="Ex:1"></td>

                      <td class="wtmem_tk_advanced" style="<?php echo esc_attr($advanced_column_style); ?>"><input type="number" name="wtmem_tk_regular_tickets[<?php echo esc_attr($key); ?>][default_qty]" value="<?php echo esc_attr(isset($ticket['default_qty']) ? $ticket['default_qty'] : ''); ?>" placeholder="Ex:1"></td>
                      <td class="wtmem_tk_advanced" style="<?php echo esc_attr($advanced_column_style); ?>"><input type="number" name="wtmem_tk_regular_tickets[<?php echo esc_attr($key); ?>][reserve_qty]" value="<?php echo esc_attr(isset($ticket['reserve_qty']) ? $ticket['reserve_qty'] : ''); ?>" placeholder="Ex:1"></td>

                      <td class="wtmem_tk_advanced" style="<?php echo esc_attr($advanced_column_style); ?>"><input type="date" name="wtmem_tk_regular_tickets[<?php echo esc_attr($key); ?>][sale_start_date]" value="<?php echo esc_attr(isset($ticket['sale_start_date']) ? $ticket['sale_start_date'] : ''); ?>"></td>
                      <td class="wtmem_tk_advanced" style="<?php echo esc_attr($advanced_column_style); ?>"><input type="time" name="wtmem_tk_regular_tickets[<?php echo esc_attr($key); ?>][sale_start_time]" value="<?php echo esc_attr(isset($ticket['sale_start_time']) ? $ticket['sale_start_time'] : ''); ?>"></td>
      ...
      public function wtmem_save_meta_ticket_price($post_id){ 

        // Save registration enabled
        if (isset($_POST['wtmem_tk_registration_enabled'])) {
            update_post_meta($post_id, '_wtmem_tk_registration_enabled', sanitize_text_field(wp_unslash($_POST['wtmem_tk_registration_enabled'])));
        }
        
        // Save advanced toggle state
        if (isset($_POST['wtmem_tk_advanced_toggle'])) {
            update_post_meta($post_id, '_wtmem_tk_advanced_toggle', sanitize_text_field(wp_unslash($_POST['wtmem_tk_advanced_toggle'])));
        }
        ...
        if (isset($_POST['wtmem_tk_regular_tickets']) && is_array($_POST['wtmem_tk_regular_tickets'])) {
            $raw_tickets = wp_unslash($_POST['wtmem_tk_regular_tickets']);

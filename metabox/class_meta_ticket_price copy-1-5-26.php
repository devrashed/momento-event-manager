...
class Class_meta_ticket_price_aaaa{ 
      ...
      public function webcu_ticket_price_sections($post) {  
           ...
           // Get saved data
            ...
            $extras = get_post_meta($post->ID, '_webcu_tk_extras', true);
                    ...
                    <tr class="webcu_tk_ticket-row">
                      ...
                      <td>
                        <select name="webcu_tk_tickets[<?php echo esc_attr($key); ?>][qty_box]">
                          <option <?php selected($ticket['qty_box'], 'Input Box'); ?>><?php echo esc_html__('Input Box', 'mega-event-manager') ?></option>
                          <option <?php selected($ticket['qty_box'], 'Dropdown'); ?>><?php echo esc_html__('Dropdown', 'mega-event-manager') ?></option>
                        </select>
                      </td>
              ...
              <tbody>
                <?php if (!empty($extras)) : ?>
                  <?php foreach ($extras as $key => $extra) : ?>
                    <tr>
                      <td><input type="text" name="webcu_tk_extras[<?php echo esc_attr($key); ?>][name]" value="<?php echo esc_attr($extra['name']); ?>" placeholder="Name" /></td>
                      <td><input type="number" name="webcu_tk_extras[<?php echo esc_attr($key); ?>][price]" value="<?php echo esc_attr($extra['price']); ?>" placeholder="Price" /></td>
                      ...
                      <td>
                        <select name="webcu_tk_extras[<?php echo esc_attr($key); ?>][qty_box]">
                          <option <?php selected($extra['qty_box'], 'Input Box'); ?>><?php echo esc_html__('Input Box', 'mega-event-manager') ?></option>
                          <option <?php selected($extra['qty_box'], 'Dropdown'); ?>><?php echo esc_html__('Dropdown', 'mega-event-manager') ?></option>
                        </select>
                      </td>
      ...
      public function webcu_save_meta_ticket_price($post_id){ 
        ...
        if (isset($_POST['webcu_tk_tickets']) && is_array($_POST['webcu_tk_tickets'])) {
            $raw_tickets = wp_unslash($_POST['webcu_tk_tickets']); // <-- FIX
            $tickets = array();
        ...
        if (isset($_POST['webcu_tk_extras']) && is_array($_POST['webcu_tk_extras'])) {
              $extraticket = wp_unslash($_POST['webcu_tk_extras']);
              $extras = array();
              foreach ($extraticket as $key => $extra) {
                  $extras[$key] = array(
              ...
              update_post_meta($post_id, '_webcu_tk_extras', $extras);
          } else {

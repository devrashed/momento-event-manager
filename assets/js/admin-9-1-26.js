...
jQuery(function ($) {
	...
	jQuery('#wtmem_tk_addTicket').on('click', function (e) {
		e.preventDefault();
		var cols = jQuery('#wtmem_tk_ticketTable thead th').length;
		var timestamp = Date.now();
		var row = '<tr class=\"wtmem_tk_ticket-row\">' +
			'<td><input type=\"text\" name=\"wtmem_tk_tickets[' + timestamp + '][name]\" placeholder=\"Ticket\"></td>' +
			'<td><input type=\"text\" name=\"wtmem_tk_tickets[' + timestamp + '][desc]\" placeholder=\"Short description\"></td>' +
			'<td><input type=\"number\" name=\"wtmem_tk_tickets[' + timestamp + '][price]\" value=\"0\"></td>' +
			'<td><input type=\"number\" name=\"wtmem_tk_tickets[' + timestamp + '][capacity]\" value=\"0\"></td>' +
			'<td class=\"wtmem_tk_advanced\"><input type=\"number\" name=\"wtmem_tk_tickets[' + timestamp + '][default_qty]\" placeholder=\"Ex:1\"></td>' +
			'<td class=\"wtmem_tk_advanced\"><input type=\"number\" name=\"wtmem_tk_tickets[' + timestamp + '][reserve_qty]\" placeholder=\"Ex:1\"></td>' +
			'<td class=\"wtmem_tk_advanced\"><input type=\"date\" name=\"wtmem_tk_tickets[' + timestamp + '][sale_start_date]\"></td>' +
			'<td class=\"wtmem_tk_advanced\"><input type=\"time\" name=\"wtmem_tk_tickets[' + timestamp + '][sale_start_time]\"></td>' +
			'<td class=\"wtmem_tk_advanced\"><input type=\"date\" name=\"wtmem_tk_tickets[' + timestamp + '][sale_end_date]\"></td>' +
			'<td class=\"wtmem_tk_advanced\"><input type=\"time\" name=\"wtmem_tk_tickets[' + timestamp + '][sale_end_time]\"></td>' +
			'<td><select name=\"wtmem_tk_tickets[' + timestamp + '][qty_box]\"><option>Input Box</option><option>Dropdown</option><option>Hidden</option></select></td>' +
	...
	jQuery('#wtmem_tk_addExtra').on('click', function (e) {
		...
		var timestamp = Date.now();
		var row = '<tr>' +
			'<td><input type=\"text\" name=\"wtmem_tk_extras[' + timestamp + '][name]\" placeholder=\"Name\" /></td>' +
			'<td><input type=\"number\" name=\"wtmem_tk_extras[' + timestamp + '][price]\" placeholder=\"Price\" /></td>' +
			'<td><input type=\"number\" name=\"wtmem_tk_extras[' + timestamp + '][available_qty]\" placeholder=\"Available Qty\" /></td>' +
			'<td><select name=\"wtmem_tk_extras[' + timestamp + '][qty_box]\"><option>Input Box</option><option>Dropdown</option></select></td>' +
		...
		const newBox = `
            ...
            <div class="wtmem_re_form-row">
                    ...
                    <div style="display:flex;align-items:center;gap:10px;">
                        <input class="wtmem_re_timing" name="timing_${counter}" type="text" value="168" /> 
